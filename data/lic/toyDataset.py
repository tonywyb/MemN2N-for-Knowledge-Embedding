...
class toyDataset(data.Dataset):
    def __init__(self, dataset_dir, memory_size=50, mode="train"):
        ...
        dict_path = os.path.join(dataset_dir, "toy_word_idx.pkl")
        ...
        if not os.path.exists(dict_path):
            ...
            self.vocab_len = len(vocab_dict) if len(vocab_dict) <= 10000 - len(token_lis) else 10000 - len(token_lis)
            self.vocab_len += len(token_lis)
            self.word_idx = dict(token_lis)
            for i, word in enumerate(self.vocab[:self.vocab_len], 4):
